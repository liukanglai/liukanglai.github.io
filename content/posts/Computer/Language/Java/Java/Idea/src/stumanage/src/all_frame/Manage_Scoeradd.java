...
class Manage_Scoeradd extends JFrame implements ActionListener{
    ...
    public void AddScore(String str_sname,String str_stuid,String str_Couid,String str_cname,String str_score)
    {
    	try{
			Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDrive");//注册成功
		}
			...
			rs=sql.executeQuery("select * from DB_Student where (StuID= '"+str_stuid+"') ");
			 ...
			 {  
				  if(rs.getString("StuName").trim().equals(str_sname))
				   {
					 rs=sql.executeQuery("select * from DB_Course where (CourseID= '"+str_Couid+"') ");
				  ...
				  if(rs.next())
				   {   
					 ...
					 try{
							String str_lock="U";
							int ok=sql.executeUpdate("insert into DB_Grade  values('"+str_stuid+"','"+str_Couid+"','"+str_score+"','"+str_lock+"')");
				        if(ok==0)
				        {            	
					  ...
					  else{JOptionPane.showMessageDialog(null,"课程号与课程名不匹配，请重新输入!","添加记录",JOptionPane.WARNING_MESSAGE);}  
				 }
				 else
				 {JOptionPane.showMessageDialog(null,"课程号不对,请添加该课程!","添加记录",JOptionPane.WARNING_MESSAGE);}
			   }
				 else{JOptionPane.showMessageDialog(null,"学生姓名与学号不匹配，请重新输入!","添加记录",JOptionPane.WARNING_MESSAGE);}
			 }
			 else
			 {
				 JOptionPane.showMessageDialog(null,"学号不对，请添加该学生!","添加记录",JOptionPane.WARNING_MESSAGE);
			 }
	...
	public void actionPerformed(ActionEvent e){
		...
		if(Text_Stuid.getText().trim().length()==0)
			JOptionPane.showMessageDialog(null,"学号不能为空!","添加记录",JOptionPane.WARNING_MESSAGE);
		else if(Text_Name.getText().trim().length()==0)
			JOptionPane.showMessageDialog(null,"姓名不能为空!","添加记录",JOptionPane.WARNING_MESSAGE);
		else
		    AddScore(Text_Name.getText(),Text_Stuid.getText(),Text_Couid.getText(),Text_Cname.getText(),Text_Score.getText());
	}	
}
