...
public class Tea_Glist_my extends JFrame implements ActionListener{
	...
	float ave1=0;
	String ave;
	int Passnum=0,Unpassnum=0;
	String TeaID;
	String teacher="";
	
	Tea_Glist_my(String TeaID){
		this.TeaID=TeaID;
	...
	public void screen(String str_cou,String str_class,String str_name,String str_num)
	{
			...
			if(str_cou.equals("不限"))
			{
				if(str_class.equals("不限"))
				{
					try{   
				        ...
				        rs.beforeFirst();
			            while(rs.next()){
			            	if(rs.getString("TeacherID").trim().compareTo(TeaID)==0)
			            	{
			            		teacher=rs.getString("TeacherName").trim();
			            		a[i][0]=i+1;
				            	...
				            	String ave=rs.getString("LastScore").trim();
				            	ave1+=Float.parseFloat(ave);
				            	i++;
				            	if(Float.parseFloat(ave)>=60)
				            		Passnum++;
				            	else Unpassnum++;
			            	}
			            }
			...
			else
			{
				...
				else
				{
					try{   
						...
						rs=sql.executeQuery("筛选成绩 "+str_cou+"'");
			            rs.last();
			            n=rs.getRow();
			   	        a=new Object[n][7];	
			   	        b=new Object[n][7];
        ...
        Name(str_name);
        Num(str_num);
        JLabel teacher1=new JLabel("教师："+teacher+"你好！");
        pNorth.add(teacher1);
        
        LName=new JLabel("按名字");
        pNorth.add(LName);
        pNorth.add(Name);
        ...
        pNorth.add(enter);
        pNorth.add(update);
        JLabel ave2=new JLabel("平均分:");
        JLabel ave3=new JLabel(ave1+"");
        pNorth.add(ave2);
        pNorth.add(ave3);
        
        JLabel pass=new JLabel("及格人数:");
        JLabel pass1=new JLabel(Passnum+"");
        pNorth.add(pass);
        pNorth.add(pass1);
        
        JLabel Unpass=new JLabel("不及格人数:");
        JLabel Unpass1=new JLabel(Unpassnum+"");
        pNorth.add(Unpass);
        pNorth.add(Unpass1);
