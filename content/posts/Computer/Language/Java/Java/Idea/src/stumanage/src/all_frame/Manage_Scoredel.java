...
import javax.swing.JTextField;
 ...
 class Manage_Scoredel extends JFrame implements ActionListener{
				  ...
				  if(rs.next())
				   {   
					 ...
					 try{
							...
							{
							int ok=sql.executeUpdate("delete  from DB_Grade where (StuID= '"+str_stuid+"' and CourseID='"+str_Couid+"')");
				        if(ok==0)
				        {            	
				        ...
				        else
				        {
							...
							{
								JOptionPane.showMessageDialog(null,"成绩已锁定，请联系管理员","删除记录",JOptionPane.WARNING_MESSAGE);
							}
						}
					  ...
					  else{JOptionPane.showMessageDialog(null,"课程号与课程名不匹配，请重新输入!","删除记录",JOptionPane.WARNING_MESSAGE);}  
				 }
				 else
				 {JOptionPane.showMessageDialog(null,"无此课程号,请重新输入!","删除记录",JOptionPane.WARNING_MESSAGE);}
			   }
				 else{JOptionPane.showMessageDialog(null,"学号与姓名不怕匹配，请重新输入!","删除记录",JOptionPane.WARNING_MESSAGE);}
			 }
			 else
			 {
				 JOptionPane.showMessageDialog(null,"无此学生，请重新输入!","删除记录",JOptionPane.WARNING_MESSAGE);
			 }
